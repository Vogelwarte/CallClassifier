...
def parse_commandline():
	...
	parser.add_argument("-i", "--input", help="Path to the file or folder of the manual annotation in the *.Table.1.selections.txt format", default=".")
	parser.add_argument("-r", "--audio-root-dir", help="Path to the root directory of the audio files. Default=current working dir.", default=".")
	parser.add_argument("-o", "--output-directory", help="Path to the output directory. If doesn't exist, it will be created.", default="./chunks")
	parser.add_argument("-a", "--annotation", help="Uses only these lines from the input file(s) that contain this string", default="")
	return parser.parse_args()


def createChunks(cd: Table1SelectionRecord, audio_file: Path, name_stem: str,  output_dir: Path, annotation: str) -> bool:
...
def createChunkFile(file_path: Path, rts:RelativeTimeSegment, audio_file: Path) -> bool:
	if file_path.exists():
		...
		return False
	sys_call:str = "sox \""+str(audio_file)+"\" \"" + str(file_path)+ "\" trim "+ str(rts.begin_time)+" "+str(rts.duration())+" "
	print(sys_call);
